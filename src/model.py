...
from .networks import discriminators, encoders, generators, funcs
...
class DRIT(nn.Module):
  def __init__(self, opts):
    ...
    # discriminators
    if opts.dis_scale > 1:
      self.disA = discriminators.MultiScaleDis(opts.input_dim_a, opts.dis_scale, norm=opts.dis_norm, sn=opts.dis_spectral_norm)
      self.disB = discriminators.MultiScaleDis(opts.input_dim_b, opts.dis_scale, norm=opts.dis_norm, sn=opts.dis_spectral_norm)
      self.disA2 = discriminators.MultiScaleDis(opts.input_dim_a, opts.dis_scale, norm=opts.dis_norm, sn=opts.dis_spectral_norm)
      self.disB2 = discriminators.MultiScaleDis(opts.input_dim_b, opts.dis_scale, norm=opts.dis_norm, sn=opts.dis_spectral_norm)
    else:
      self.disA = discriminators.Dis(opts.input_dim_a, norm=opts.dis_norm, sn=opts.dis_spectral_norm)
      self.disB = discriminators.Dis(opts.input_dim_b, norm=opts.dis_norm, sn=opts.dis_spectral_norm)
      self.disA2 = discriminators.Dis(opts.input_dim_a, norm=opts.dis_norm, sn=opts.dis_spectral_norm)
      self.disB2 = discriminators.Dis(opts.input_dim_b, norm=opts.dis_norm, sn=opts.dis_spectral_norm)
    self.disContent = discriminators.Dis_content()

    # encoders
    # combining content representation and attribute ve
    self.enc_c = encoders.E_content(opts.input_dim_a, opts.input_dim_b)
    if self.concat:
      # concatenation
      self.enc_a = encoders.E_attr_concat(opts.input_dim_a, opts.input_dim_b, self.nz, \
          norm_layer=None, nl_layer=funcs.get_non_linearity(layer_type='lrelu'))
    else:
      # featurewise transformation
      self.enc_a = encoders.E_attr(opts.input_dim_a, opts.input_dim_b, self.nz)

    # generator
    if self.concat:
      self.gen = generators.G_concat(opts.input_dim_a, opts.input_dim_b, nz=self.nz)
    else:
      self.gen = generators.G(opts.input_dim_a, opts.input_dim_b, nz=self.nz)
  ...
  def initialize(self):
    self.disA.apply(funcs.gaussian_weights_init)
    self.disB.apply(funcs.gaussian_weights_init)
    self.disA2.apply(funcs.gaussian_weights_init)
    self.disB2.apply(funcs.gaussian_weights_init)
    self.disContent.apply(funcs.gaussian_weights_init)
    self.gen.apply(funcs.gaussian_weights_init)
    self.enc_c.apply(funcs.gaussian_weights_init)
    self.enc_a.apply(funcs.gaussian_weights_init)

  def set_scheduler(self, opts, last_ep=0):
    self.disA_sch = funcs.get_scheduler(self.disA_opt, opts, last_ep)
    self.disB_sch = funcs.get_scheduler(self.disB_opt, opts, last_ep)
    self.disA2_sch = funcs.get_scheduler(self.disA2_opt, opts, last_ep)
    self.disB2_sch = funcs.get_scheduler(self.disB2_opt, opts, last_ep)
    self.disContent_sch = funcs.get_scheduler(self.disContent_opt, opts, last_ep)
    self.enc_c_sch = funcs.get_scheduler(self.enc_c_opt, opts, last_ep)
    self.enc_a_sch = funcs.get_scheduler(self.enc_a_opt, opts, last_ep)
    self.gen_sch = funcs.get_scheduler(self.gen_opt, opts, last_ep)
  ...
  def test_forward(self, image, a2b=True, ori=False):
    if ori:
      self.z_random = torch.zeros(image.size(0), self.nz).cuda(self.gpu)
    else:
  ...
  def test_forward_transfer(self, image_a, image_b, a2b=True, ori=False):
